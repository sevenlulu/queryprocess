...
class GroceryTextPreProcessor(object):
    ...
    @staticmethod
    #加入分词字典和停用词字典在textgrocery 默认的结巴分词中    
    def _default_tokenize(text):
        jieba.load_userdict(os.path.abspath(os.path.curdir)+"/data/customdic.txt")
        jieba.load_userdict(os.path.abspath(os.path.curdir)+"/data/singer_custom_dict.txt")
        segs = jieba.cut(text, cut_all = True)
        stopwords_list = []
        with open(os.path.abspath(os.path.curdir)+"/data/stopwords.txt") as stopwords_file:
